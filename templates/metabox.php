	...
	<tr>
		...
		<td colspan="8">
			<textarea name="tsoh_note" id="tsoh_note" cols="40" rows="3"
					  ...
					  style="width:90%;"><?= esc_textarea( get_post_meta( $post->ID, "_tsoh_holidy_note", true ) ); ?></textarea>
		</td>
	</tr>
	</tfoot>
	<tbody>
	<?php $counter = 0;
	foreach ( $this->model->get_timetable( $post->ID ) as $start => $time ): if ( count( $time ) < 3 ) {
		continue;
	}
		$counter ++; ?>
